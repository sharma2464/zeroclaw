use crate::agent::dispatcher::{ParsedToolCall, XmlToolDispatcher};
                ...
                Ok(resp) => {
                    ...
                    if calls.is_empty() {
                        let (fallback_text, fallback_calls) =
                            XmlToolDispatcher::parse_xml_tool_calls(&response_text);
...
pub async fn run(
    ...
    // ── Build system prompt from workspace MD files (ZeroClaw framework) ──
